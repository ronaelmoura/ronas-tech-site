  ...
  return <section className={styles.section} id="produtos-digitais" aria-labelledby="digital-products-title">
    <div className={styles.container}>
      ...
      <div className={styles.grid}>
        ...
        {spreadsheetProducts.map((product) => <a className={`${styles.card} ${styles.available}`} href={product.path} key={product.path}><div className={`${styles.cover} ${styles.generatedCover}`}><img src={product.image} alt={`Capa de ${product.title}`} loading="lazy" /></div><div className={styles.cardBody}><span className={styles.category}>{product.category}</span><h3>{product.title}</h3><p>{product.description}</p><div className={styles.cardBottom}><strong>Conhecer produto</strong><b>Disponível</b></div></div></a>)}
      </div>
