  ...
  return <section className={styles.section} id="produtos-digitais" aria-labelledby="digital-products-title">
    <div className={styles.container}>
      ...
      <div className={styles.grid}>
        <a className={`${styles.card} ${styles.available}`} href="/produtos-digitais/kit-financeiro-mei"><div className={`${styles.cover} ${styles.generatedCover}`}><img src="/capa-kit-financeiro-mei-v2.png" alt="Kit Financeiro Inteligente para MEI" loading="lazy" /></div><div className={styles.cardBody}><span className={styles.category}>Finanças para MEI</span><h3>Kit Financeiro Inteligente para MEI</h3><p>Controle financeiro, contas e precificação para pequenos negócios.</p><div className={styles.cardBottom}><strong>Conhecer produto</strong><b>R$ 37,90</b></div></div></a>
        ...
        {spreadsheetProducts.map((product) => <a className={`${styles.card} ${styles.available}`} href={product.path} key={product.path}><div className={`${styles.cover} ${styles.generatedCover}`}><img src={product.image} alt={`Capa de ${product.title}`} loading="lazy" /></div><div className={styles.cardBody}><span className={styles.category}>{product.category}</span><h3>{product.title}</h3><p>{product.description}</p><div className={styles.cardBottom}><strong>Conhecer produto</strong><b>{product.price}</b></div></div></a>)}
      </div>
