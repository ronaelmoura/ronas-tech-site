...
function Hero() {
  ...
  return <section id="inicio" className={styles.hero} aria-labelledby="hero-title"><div className={styles.glow} aria-hidden="true" /><div className={styles.container}><div className={styles.content}><p className={`${styles.eyebrow} hero-eyebrow-anim`}><span aria-hidden="true" />Suporte remoto para Windows · todo o Brasil</p><div className="hero-mask"><h1 id="hero-title" className={styles.title}>Seu computador volta a <span>acompanhar sua rotina.</span></h1></div><p className={`${styles.description} hero-desc-anim`}>PC lento, travando, com anúncios ou programa que não abre? Eu analiso o problema, explico sem termos complicados e confirmo o valor antes de começar.</p><div className={`${conversion.commercial} hero-actions-anim`}><span><small>Primeiro contato</small><strong>Triagem gratuita</strong></span><span><small>Diagnóstico técnico</small><strong>A partir de R$ 39</strong></span><span><small>Atendimento</small><strong>Todos os dias · 09h–00h</strong></span></div><div className={`${styles.actions} hero-actions-anim`}><a ref={primaryRef} className={styles.primaryButton} href="#loja">Escolher meu atendimento</a><a ref={secondaryRef} className={styles.secondaryButton} data-ronas-cta="hero_cta" href={whatsappUrl} target="_blank" rel="noopener noreferrer" onClick={openWhatsApp}>Explicar meu problema · grátis</a></div><div className={`${styles.availability} hero-actions-anim`}><AvailabilityBadge /></div><ul className={`${styles.trustList} hero-actions-anim`}><li>Você acompanha tudo pela tela</li><li>Atendimento direto com Ronael</li><li>Valor combinado antes do serviço</li></ul></div><div className={`${styles.visual} hero-visual-anim`} role="img" aria-label="Painel ilustrativo de diagnóstico remoto acompanhado"><DiagnosticConsole /></div></div></section>
}
